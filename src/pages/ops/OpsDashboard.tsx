...
function Kpi({ label, value, tone = 'default', icon: Icon }: { label: string; value: string | number; tone?: 'default' | 'warning' | 'danger' | 'success'; icon: React.ElementType }) {
  ...
  return <div className="card"><div className="flex items-center justify-between mb-2"><Icon className="w-4 h-4 text-gray-400" /></div><p className={cn('text-2xl font-heading font-bold', toneClass)}>{value}</p><p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{label}</p></div>;
}

const NAV_ITEMS = [
  { to: '/ops/workspace', label: 'Industry Control Centre', description: 'Your industry-specific operating workspace', icon: LayoutDashboard, moduleKey: 'workspace' },
...
const ADMIN_NAV_ITEMS = [{ to: '/ops/admin/assets', label: 'Asset Registry', description: 'Sites, business units, asset types, assets', icon: Car, permission: 'assets.create' }];
...
export default function OpsDashboard() {
  ...
  const visibleNavItems = NAV_ITEMS.filter((item) => item.moduleKey === 'workspace' || item.moduleKey === 'work_orders' || item.moduleKey === 'intelligence' || isModuleEnabled(org?.enabled_modules, item.moduleKey)).sort((a, b) => {
    ...
    return (ai === -1 ? industryConfig.priorityModules.length : ai) - (bi === -1 ? industryConfig.priorityModules.length : bi);
  });
  const visibleAdminItems = ADMIN_NAV_ITEMS.filter((item) => can(item.permission)).map((item) => item.to === '/ops/admin/assets' ? { ...item, label: `${industryConfig.assetLabelPlural} Registry`, description: `Sites, business units, ${industryConfig.assetLabelSingular.toLowerCase()} types, ${industryConfig.assetLabelPlural.toLowerCase()}` } : item);
  ...
  if (!orgLoading && !org) return <div className="px-4 pt-6"><EmptyState icon={GaugeIcon} title="No organisation found" description="Your account isn't linked to an organisation yet. Contact your administrator." /></div>;
  ...
  return <div className="px-4 pt-6 pb-6"><div className="mb-6"><p className="text-gray-500 dark:text-gray-400 text-sm flex items-center gap-1.5"><GaugeIcon className="w-4 h-4" /> {org?.organisation_name ?? 'Operations'} {org?.industry_mode && org.industry_mode !== 'general' && <span className="ml-1 px-1.5 py-0.5 rounded text-[10px] font-semibold bg-brand-50 text-brand dark:bg-charcoal-light uppercase tracking-wide">{INDUSTRY_LABELS[org.industry_mode]}</span>}</p><h1 className="font-heading font-bold text-2xl">Operations Control</h1><p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{industryConfig.tagline}</p></div><div className="grid grid-cols-2 gap-3 mb-6">{isLoading ? [1,2,3,4].map((i) => <SkeletonCard key={i} />) : kpiKeys.map((key) => { const def = KPI_DEFINITIONS[key]; return def ? <Kpi key={key} label={def.label} value={def.value} tone={def.tone} icon={def.icon} /> : null; })}</div><h2 className="font-heading font-bold text-sm text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2 px-1">Workspace</h2>{visibleNavItems.length === 0 ? <EmptyState icon={GaugeIcon} title="No modules enabled" description="Your organisation admin hasn't turned on any Ops modules yet." /> : <div className="flex flex-col gap-2">{visibleNavItems.map(({ to, label, description, icon: Icon }) => <Link key={to} to={to} className="card flex items-center gap-3 active:scale-[0.98] transition-transform"><div className="w-10 h-10 rounded-xl bg-brand-50 dark:bg-charcoal-light flex items-center justify-center shrink-0"><Icon className="w-5 h-5 text-brand" /></div><div className="flex-1 min-w-0"><p className="font-semibold text-sm">{label}</p><p className="text-xs text-gray-500 dark:text-gray-400 truncate">{description}</p></div><ChevronRight className="w-4 h-4 text-gray-300 shrink-0" /></Link>)}</div>}{visibleAdminItems.length > 0 && <><h2 className="font-heading font-bold text-sm text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2 px-1 mt-6">Admin</h2><div className="flex flex-col gap-2">{visibleAdminItems.map(({ to, label, description, icon: Icon }) => <Link key={to} to={to} className="card flex items-center gap-3 active:scale-[0.98] transition-transform"><div className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-charcoal-light flex items-center justify-center shrink-0"><Icon className="w-5 h-5 text-gray-500 dark:text-gray-300" /></div><div className="flex-1 min-w-0"><p className="font-semibold text-sm">{label}</p><p className="text-xs text-gray-500 dark:text-gray-400 truncate">{description}</p></div><ChevronRight className="w-4 h-4 text-gray-300 shrink-0" /></Link>)}</div></>}</div>;
}
