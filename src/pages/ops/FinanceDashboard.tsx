...
import { usePurchaseOrders, useWorkOrders } from '@/hooks/useAfriops';
...
export default function FinanceDashboard() {
  ...
  const committedSpend = pos.filter((p) => !['draft', 'cancelled'].includes(p.status)).reduce((sum, p) => sum + (p.total_amount ?? 0), 0);
  const realisedMaintenance = work.filter((w) => w.actual_cost != null).reduce((sum, w) => sum + (w.actual_cost ?? 0), 0);
  const pendingApproval = pos.filter((p) => p.status === 'submitted');

  return <div className="px-4 pt-6 pb-8 max-w-6xl mx-auto">
    <header className="mb-6"><p className="text-xs uppercase tracking-wide font-semibold text-gray-500">{org?.organisation_name} · Finance</p><h1 className="font-heading font-bold text-2xl mt-1">Cost &amp; Commitments</h1><p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Track operational spend, purchase commitments and realised work-order cost.</p></header>
    {(poLoading || woLoading) ? <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">{[1,2,3,4].map((i) => <SkeletonCard key={i} />)}</div> : <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
      <div className="card"><p className="text-2xl font-heading font-bold">{formatCurrencyZAR(committedSpend)}</p><p className="text-xs text-gray-500 mt-1">Committed PO spend</p></div>
      <div className="card"><p className="text-2xl font-heading font-bold">{formatCurrencyZAR(realisedMaintenance)}</p><p className="text-xs text-gray-500 mt-1">Recorded work cost</p></div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <section className="card"><div className="flex items-center gap-2 mb-3"><ShoppingCart className="w-4 h-4 text-brand"/><h2 className="font-heading font-bold">Purchase commitments</h2></div>
        {openPos.length === 0 ? <p className="text-xs text-gray-500">No open purchase orders.</p> : <div className="space-y-2">{openPos.slice(0,6).map((po) => <Link key={po.id} to="/ops/procurement" className="block rounded-lg border border-gray-100 dark:border-charcoal-light p-3"><div className="flex items-center justify-between gap-2"><p className="text-sm font-medium">PO {po.id.slice(0,8)}</p><span className="text-xs font-semibold">{formatCurrencyZAR(po.total_amount ?? 0)}</span></div><p className="text-xs text-gray-500 mt-1">{po.status.replace('_',' ')} · {formatDate(po.created_at)}</p></Link>)}</div>}
      </section>
