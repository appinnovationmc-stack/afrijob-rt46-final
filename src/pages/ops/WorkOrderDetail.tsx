...
export default function WorkOrderDetail() {
  ...
  if (isLoading) {
    return <div className="px-4 pt-6 pb-24"><SkeletonCard /><SkeletonCard /></div>;
  }
  ...
  const categoryLabel = industryConfig.categoryLabels?.[wo.category] ?? WORK_ORDER_CATEGORY_LABELS[wo.category];
  ...
  return (
    <div className="px-4 pt-6 pb-24">
      <Link to="/ops/work-orders" className="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 mb-3"><ArrowLeft className="w-4 h-4" /> Work Orders</Link>
      <div className="flex items-start justify-between gap-2 mb-1">
        <h1 className="font-heading font-bold text-xl flex-1">{wo.description || categoryLabel}</h1>
        <EnumStatusChip status={wo.status} styles={WORK_ORDER_STATUS_STYLES} labels={WORK_ORDER_STATUS_LABELS} />
      </div>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{WORK_ORDER_SOURCE_LABELS[wo.source_system] ?? wo.source_system} · {categoryLabel}</p>

      <div className="flex items-center gap-2 mb-4">
        <span className={cn('inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold', priorityMeta.className)}>{priorityMeta.label} priority</span>
        {wo.actual_cost != null && <span className="text-sm font-semibold">{formatCurrencyZAR(wo.actual_cost)}</span>}
      </div>
      ...
      {!parts?.length ? <EmptyState icon={Boxes} title="No parts logged" description="No parts have been recorded against this work order yet." /> : (
        <div className="card space-y-2">
          {parts.map((p: any) => (
            <div key={p.id} className="flex items-center justify-between text-sm">
              <div><p className="font-medium">{p.description}</p><p className="text-xs text-gray-500 dark:text-gray-400">{PART_SOURCE_LABELS[p.source] ?? p.source} · {p.quantity} × {p.unit_cost != null ? formatCurrencyZAR(p.unit_cost) : '—'} · {formatDate(p.created_at)}</p></div>
          ...
          <div className="flex items-center justify-between text-sm font-semibold"><span>Parts total</span><span>{formatCurrencyZAR(partsTotal)}</span></div>
        </div>
      )}
      ...
      {!slaBreaches?.length ? <EmptyState icon={Timer} title="No SLA breaches" description="This work order has not breached any SLA target." /> : (
        <div className="space-y-2">{slaBreaches.map((b) => <div key={b.id} className={cn('card !py-3', !b.acknowledged_at && 'border-danger/40')}><div className="flex items-start justify-between gap-2 mb-1"><p className="font-medium text-sm flex-1 capitalize">{b.metric} breach</p>{!b.acknowledged_at && <span className="text-xs font-semibold text-danger">Unacknowledged</span>}</div><p className="text-xs text-gray-500 dark:text-gray-400">{b.minutes_over} min over target · Breached {formatDate(b.breached_at)}</p></div>)}</div>
      )}
      ...
      {!incidents?.length ? <EmptyState icon={AlertTriangle} title="No linked incidents" description="No incidents have been linked to this work order." /> : <div className="space-y-2">{incidents.map((i) => <div key={i.id} className="card !py-3"><p className="font-medium text-sm">{i.category}</p><p className="text-xs text-gray-500 dark:text-gray-400 capitalize">{i.severity} severity · {i.status} · {formatDate(i.occurred_at)}</p></div>)}</div>}
      ...
      {!documents?.length ? <EmptyState icon={FileText} title="No documents" description="No documents have been attached to this work order." /> : <div className="space-y-2">{documents.map((d) => <div key={d.id} className="card !py-3 flex items-center justify-between gap-2"><div className="min-w-0"><p className="font-medium text-sm truncate">{d.doc_type}</p>{d.expiry_date && <p className="text-xs text-gray-500 dark:text-gray-400">Expires {formatDate(d.expiry_date)}</p>}</div><ComplianceStatusChip status={d.status} /></div>)}</div>}
      ...
      {!auditLog?.entries.length ? <EmptyState icon={History} title="No audit history" description="No changes have been recorded against this work order." /> : <div className="space-y-2">{auditLog.entries.map((entry) => <div key={entry.id} className="card !py-3"><p className="font-medium text-sm">{AUDIT_ACTION_LABELS[entry.action] ?? entry.action}</p><p className="text-xs text-gray-500 dark:text-gray-400">{entry.actor?.full_name ?? 'System'} · {formatDate(entry.created_at)}</p></div>)}</div>}
    </div>
  );
}
