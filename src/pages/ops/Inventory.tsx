...
export default function Inventory() {
  const { data: items, isLoading } = useInventoryItems(); const [showNew, setShowNew] = useState(false); const [movementItem, setMovementItem] = useState<InventoryItem | null>(null);
  return <div className="px-4 pt-6 pb-24"><h1 className="font-heading font-bold text-2xl mb-1">Inventory</h1><p className="text-sm text-gray-500 dark:text-gray-400 mb-5">Stock on hand across sites. Movements are the only way to change quantity — never edited directly.</p>
    {isLoading ? <div className="space-y-3">{[1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div> : !items?.length ? <EmptyState icon={Boxes} title="No inventory items yet" description="Add your first item to start tracking stock." /> : <div className="space-y-3">{items.map((item) => { const low = isBelowReorderPoint(item); return <div key={item.id} className="card"><div className="flex items-start justify-between mb-2"><div><p className="font-semibold">{item.name}</p><p className="text-xs text-gray-500 dark:text-gray-400">{item.sku ? `#${item.sku} · ` : ''}{item.category ?? 'Uncategorised'}</p></div>{low && <span className="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold bg-danger/15 text-danger shrink-0">Low stock</span>}</div>
      <div className="flex items-baseline gap-2 mb-3"><span className={cn('text-xl font-heading font-bold', low && 'text-danger')}>{item.quantity_on_hand}</span><span className="text-sm text-gray-500 dark:text-gray-400">{item.unit}</span>{item.reorder_point !== null && <span className="text-xs text-gray-400 ml-auto">reorder at {item.reorder_point}</span>}</div>{item.unit_cost !== null && <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">Unit cost: {formatCurrencyZAR(item.unit_cost)}</p>}
      <div className="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-800"><button className="btn-secondary flex-1 text-sm !py-2 flex items-center justify-center gap-1.5" onClick={() => setMovementItem(item)}><ArrowDownCircle className="w-4 h-4" /> Receive</button><button className="btn-secondary flex-1 text-sm !py-2 flex items-center justify-center gap-1.5" onClick={() => setMovementItem(item)}><ArrowUpCircle className="w-4 h-4" /> Issue</button></div>
    </div>; })}</div>}
    <FAB onClick={() => setShowNew(true)} label="New item" />{showNew && <NewItemModal onClose={() => setShowNew(false)} />}{movementItem && <MovementModal item={movementItem} onClose={() => setMovementItem(null)} />}
  </div>;
}
